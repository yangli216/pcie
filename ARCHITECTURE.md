...
普通语音结果页的最小化恢复复用同就诊编辑快照：快照记录 `consultationRoundId`，来源仍为 `llm` 的重建仅允许恢复同一非空轮次；显式 `cache` 入口沿用同就诊、同自然日缓存规则。`voiceEditorSnapshotRestoration.ts` 负责恢复资格与异步恢复保护，快照恢复和 Vue watcher 收敛前不得开放自动治疗生成。恢复空治疗数组时也保留已完成的诊断键，防止有效空结果重算；药房与库存仍按原规则核对。普通语音卸载前补存最新编辑，患者/就诊/轮次变化及卸载后拦截迟到恢复。此规则仅用于 `voice`，慢病复诊、症状问诊与报告回诊保留原入口与状态机。
...
注意事项复诊时限由 `features/clinical-result/precautionsFollowUp.ts` 的纯规则统一补齐；本地注意事项 Builder、共享结果页的新结果映射和生成基线复用该规则。只使用当前诊断范围及明确的慢病渠道，缺少明确复诊时间时分别提供普通诊疗 3 天、慢病定期复查 1 个月的默认建议；已有时间和紧急就医安排优先。最终回写 Builder 保留医生提供的注意事项，不在提交时补写。作用域控制器保存补齐后的系统基线，诊断变更继续保护手工文本。
...
7. 原始操作日志只保留能定位业务路径的结构化事件：`operationTracker.ts` 负责把高噪声 UI 事件白名单化过滤，并把保留事件统一上报为 `{ module, action, title, sourceModule, scene }`；`aiTrace.ts` 则为 AI 代理补齐“哪个业务发起了这次调用”的上下文，`llm.ts` 把其中的 `operationAction / operationTitle` 连同 `traceId / scene / sourceModule` 发给 `/v1/ai/chat`，使服务端实测耗时日志能按真实业务动作检索，避免后台只看到泛化的 `ai/chat`。
...
3.1 多慢病候选不直接进入同一份病历。`chronicRefillAssessment.ts` 为候选保存稳定的“慢病分组 + 历史临床诊断”选项，并把同次多慢病、无处方诊断关联的结构化药品提取为稳定 attribution item；`ReceptionCapsule.vue` 只编排入口开合，`ChronicRefillScopeSelector.vue` 只收集医生多选慢病范围并立即提交，不渲染或等待药品归类状态。病历模型的本次主诉/现病史、结果页核查、初始诊断和用药定稿只消费医生已确认的诊疗范围。候选另保存 `historicalDiagnoses`，在范围筛选与重复筛选时保留同患者完整历史慢病；`chronicRefillRecord.ts` 将其与 HIS 既往史交给共享病史模板规则，形成独立的患者既往史基线。首屏、partial、complete 和失败回退均使用该确定性基线，模型不得覆写已知病史。历史诊断仅用于既往史，不得扩充本次诊疗范围；患者切换仍沿用原有 session/患者锚点门禁。`useDiagnosisSelection.ts` 负责共享结果页的初始勾选策略：慢病复诊渠道把 scoped candidate 生成的全部正式诊断设为已选，首个标准诊断作为主诊断、其余保留为已选次诊断；这些医生已确认的慢病诊断不受普通 AI 正式建议最多 3 项的截断。语音和症状问诊继续只默认选择一个主诊断。该策略只消费已经进入结果的正式诊断，不得恢复胶囊中未选慢病或把待鉴别方向升级为正式诊断。某次历史就诊同时含多个慢病、但 HIS 未提供处方与诊断的直接关联时，全部相关慢病均被选中才允许直接继承全部处方；部分选中时，歧义药品由 `chronicRefillPromptContext.ts` 按“规范药名 + 规格 + 候选慢病集合”合并为 `M1...Mn` 短引用，慢病使用 `C1...Cn` 短引用，同一次 `chronicRefillRecord.ts` 请求只要求模型稀疏返回 high / medium 命中项。客户端将短引用白名单还原为全部对应稳定 item id 与 condition id 后，再经 `normalizeChronicRefillMedicationAttributions()` 归一；未返回、低置信、未知引用、越权新增或流式失败的药品继续只存在于不受慢病范围裁剪的 `prescriptionHistoryVisits`，供同品历史事实核查。不存在歧义药品时，Prompt 和 NDJSON 协议均省略 `medication_scope`；存在时顺序为 `record_core → medication_scope → review_plan → recommended_medicines → record_extra → done`。库存 Prompt 只包含可直接归属药品及歧义历史药品的同品候选，并使用请求内 `S1...Sn` 引用；模型返回后由客户端恢复真实药品名称和规格。归类前不把歧义药品展示或自动选中，也不因合并归类而退回完整药房目录。生成后所有药品继续经过共享药品定稿流水线，并在结果页保持可取消、可调整，AI 归类不等于最终回写。接诊 controller 不再发起独立归类请求；生成 session token、患者锚点和当前机会统一约束全部流式事件。`medication_scope` 缺失或流中断时按无可靠归类药品安全收口，不追加第二次模型请求。

3.2 慢病生成继续保持单次模型 SSE，不按病历、核查和用药拆成多个模型请求。三者共享医生确认范围、药品归类结果和同一患者锚点，拆分会产生重复推理、版本组合和迟到结果覆盖风险；当前可并行边界仍是结果页占位与库存读取、流结束后的独立本地映射及药品定稿。为降低单次请求职责，主诉、现病史、既往史、当前用药史、通用三项核查骨架和基础查体由确定性 Builder 提供；模型的 `record_core` 只作为分区就绪标记，`review_plan` 只返回允许 ID 的疾病相关 question / description / basis patch，`record_extra` 只返回简短健康指导与基础库之外的专科查体补充。近 90 天完整就诊仍保留在客户端供周期核查，模型只接收按药品聚合的最近方案、出现次数和方案差异摘要，不重复发送逐次空主诉/空现病史。慢病请求固定使用 fast profile、`temperature=0` 和关闭思考的服务端配置；若压缩后仍需拆分模型调用，必须先设计服务端聚合结果的统一版本、失败语义和患者锚点，再修改本条单次 SSE 基线。
...
普通语音治疗目录准备由 `voiceTreatmentRecommendationGeneration.ts` 分支内编排：药品库存获取与推荐、检查 / 检验实时目录获取与联合推荐同时启动；检查目录等待、为空或失败均不阻塞药品分支回调。检查与检验仍共享一次目录请求与一次模型请求；实时目录上下文构造完成后，可用类型的轻量模型请求必须立即启动，缺失类型的错误结果应用与该请求并行，不能成为模型请求的串行门槛。目录上下文只读取标准字段、项目名以及厂商明确的组合标志字段，不序列化完整 `raw` 对象做模糊扫描。整体生成仍在全部分支及结果应用回调完成后收口，原有诊断展示依赖、药品定稿与回写门禁保持不变。耗时探针分别记录目录查询、目录上下文构造、请求参数构造、模型发起、模型等待和缺失类型结果应用，便于区分 HIS、同步计算、页面回调与模型长尾。
...
> 普通语音结构化 Prompt 采用“权威临床规则 + 单份运行时兼容约束 + 紧凑患者事实”的输入结构。患者年龄必须以完整 `ageText` 连同性别进入请求，月龄、日龄不得由药名或对话推断；已有病史为空时由客户端确定性恢复权威档案。托管 Prompt 的旧分区顺序仍由 `voiceIntentStreamProtocol.ts` 原位重排，但客户端不得再在 system 和 user 两侧重复整段诊断、路由及输出协议。患者长期记忆只提供去重后的过敏、慢病和规范药名级核对线索，不发送历史药品厂家、包装、剂量、频次和疗程。基础查体候选由 `physicalExamGuidance.ts` 根据当前病历与正式诊断本地生成；同次模型 `record_suggestions` 只补基础库之外的必要专科查体和非查体候选，不重复输出基础查体库。控制台耗时日志只增加 Prompt、分区和总输出字符计数，不记录患者文本。

> 普通语音诊断在进入共享结果页前额外经过 `features/voice-consultation/lib/ordinaryVoiceDiagnosisGuard.ts` 的纯规则门禁。`voiceIntentRecognitionStream` 的诊断分区声明 `clinicalRole`、`diagnosisKind`、`evidenceScope` 与 `currentVisitEvidenceText`：`history_only / risk_modifier` 直接退出诊断建议区，不能解释本次主诉的历史共病不再降级为待鉴别；`differential_cause` 只允许表达可解释当前主诉的病因候选。若标准库匹配后不存在病因性正式诊断，门禁可从 `current_visit / both`、high / medium、本次肯定证据充分且匹配 R 类标准编码的症状候选中最多提升一项为 `symptom_working` 正式诊断，其余 R 类症状项退出诊断建议区而不进入待鉴别。该工作诊断经 `diagnosisSuggestionPresentation.ts` 保持在正式区并由 `DiagnosisRecommendationCard.vue` 明示“症状性工作诊断”，自动诊疗路由仍限制为 `diagnostic_first` 的检查 / 检验分支；医生主动发起“基于现有信息推荐用药”时，单次独立药品请求只允许有依据的对症候选，不改变自动路由或诊断分区；明确医嘱与急诊转诊继续保持原优先级。旧 Prompt 缺少新角色字段时，门禁仍根据历史上下文、证据语义和标准 R 编码做兼容过滤与提升。规则分别在语音抽取归一和共享结果页 voice 初始化处执行，缓存 key 升级为 V3，避免旧 V1/V2 快照恢复已退出诊断区的历史共病。症状问诊与已确认范围的慢病复诊不经过此门禁。
...
| `femaleHistoryEligibility.ts` / `patientContext.ts` / `outpatientRecord.ts` / `recordConfirmedPayload.ts` 的女性病史链路 | 月经史与婚育史是独立于个人史的可选病历字段。`femaleHistoryEligibility.ts` 统一执行“女性且 14≤周岁年龄<60”资格判断，完整年龄文本中的月龄、日龄优先阻断，年龄不明也不开放；患者上下文仍保留 HIS 原始事实，但语音模型上下文、慢病结果、共享结果页展示、缓存恢复和最终回写均只消费符合资格的两类病史。符合范围时，显式字段优先，必要时从既有门诊病历对应段落提取；共享结果页固定排在个人史与家族史之间，无依据时保持空白，不建立默认模板或 AI 正常候选。最终由 `useClinicalResultWritebackScope.ts` 纳入字段级选择，`recordConfirmedPayload.ts` 仅在有内容、被选择且当前患者仍符合资格时写入。动态模板继续以本次 `targetFieldIds` 为范围权威，不因年龄规则扩充目标字段 | [src/features/clinical-result/lib/femaleHistoryEligibility.ts](src/features/clinical-result/lib/femaleHistoryEligibility.ts) / [src/utils/patientContext.ts](src/utils/patientContext.ts) / [src/features/clinical-result/outpatientRecord.ts](src/features/clinical-result/outpatientRecord.ts) / [src/features/clinical-result/recordConfirmedPayload.ts](src/features/clinical-result/recordConfirmedPayload.ts) / [src/components/VoiceConsultationNew.vue](src/components/VoiceConsultationNew.vue) |
...
| `ReceptionCapsule.vue` / `ChronicRefillScopeSelector.vue` | 接待胶囊负责患者摘要、风险和门诊机会入口编排；慢病范围与医生勾选确认由同域子组件独立承载并立即提交，父组件只转交 `ChronicRefillSelection`。历史药品归类属于后续慢病生成流内部阶段，不进入胶囊状态。风险规则和风险详情组件仍由 reception-risk 提供，App 通过 `@features/reception` 公开入口消费。患者年龄 prop 使用完整 `ageText`，原样展示“岁 / 个月 / 天”及复合月日龄，并将同一文本交给头像解析 | [src/features/reception/ui](src/features/reception/ui) |
...
| 服务 | 唯一运行路径 |
|------|-------------|
| LLM Chat | 签名 SSE/POST `/v1/ai/chat`，模型凭据由服务端持有；请求携带 AI trace 的业务动作与标题，服务端记录真实上游总耗时及流式首包耗时；客户端与服务端双重限制 `enableSearch` 只作用于 `chat-stream / chat_panel` 流式小助手请求，其他问诊与病历场景强制关闭 |
...
`features/clinical-result/lib/physicalExamGuidance.ts` 为共享 domain Builder：维护基础查体项目、场景选择、模型提示词、事实冲突与项目去重；普通语音映射、候选 controller 与慢病生成复用同一规则，既有 AI 来源标记负责展示。`physicalExamMeasurements.ts` 仅提供身高/体重/腰围正文槽位及明确数值提取，由现有生命体征 Builder 调用，不改 PHIS 五项结构化生命体征契约。

普通语音在映射同次候选时补齐基础项目；症状的原有候选请求返回后增量补齐；慢病在原 SSE 的 record_extra 返回 physicalExamSuggestions，复用共享候选结构并生成 factSuggestions，不新增模型请求或确认页。候选合并前按最新正文过滤矛盾和重复，异步期间已编辑的查体不自动追加；恢复保留医生已调整/移除状态。`clinicalRecordAnnotation.ts` 提供候选文本剥离的证据投影：展示、缓存和回写保留草稿，后续用药与病历重生成使用剥离 AI 查体候选的事实文本，避免循环把建议当成检查证据。

肺部默认书写候选按试点医生最终确认使用“双肺呼吸音粗”，继续保留 AI 来源与推理证据隔离；本次明确呼吸音结果优先，不回溯替换医生正文或已有缓存草稿。


### 初步诊断建议与其他病因鉴别（2026-09-18）

正式诊断建议表示当前证据支持、供医生确认的临床初步诊断，不要求先排除全部其他疾病。结构化 formal/high或medium 项不得仅因推荐理由包含“需排除其他疾病”“需进一步检查”降级；明确 differential、低置信、诊断名待排及目标诊断本身证据不足仍进入待鉴别。普通语音同次 diagnoses 分区在无病因性正式诊断且存在明确肯定症状时应输出至多一项症状性工作诊断，证据、标准库匹配和 R 类编码门禁保持有效；没有可用依据允许空结果，不新增串行模型补发，不把上腹不适推断成腹痛，不把 AI 查体候选当成事实。

### 普通语音控制台耗时探针（2026-09-22）

`features/voice-consultation/model/voiceTimingTracker.ts` 提供独立观测 controller，以 `performance.now()` 记录确认、窗口就绪、模型首包、NDJSON 分区、同步构建、明确医嘱匹配和治疗处理；`useVoiceTimingPresentation.ts` 在 Vue DOM 更新后观测病历/正式诊断与初始结果就绪。仅当前普通语音轮次可写入，旧请求、缓存恢复和其他渠道不得混入；模型流结束与页面/治疗完成分别计时，DOM 更新不宣称为浏览器实际绘制。

控制台统一 `[VoiceTiming]` 前缀，逐项使用纯文本输出；结束时用单条多行“截图汇总”集中显示总耗时、分区到达、关键 span 及计算耗时，不使用需手动展开的对象或表格。未记录的阶段显示“未记录”，不得当作零；相邻节点间隔不解释为并行任务独占耗时，同步计算与异步阶段以独立 span 记录。仅保留一个轮次与有限条技术指标，不记录患者标识、转写文本、诊断或药品内容，不进入回写契约、不新增 store、不增加模型请求。日志失败不得影响业务，正式构建也保留输出。

### 普通语音诊断优先输出（2026-09-22）

同次 NDJSON 固定按 `record_core → history_context → diagnoses → recommendation_plan → record_suggestions → explicit_orders → record_extra → done` 输出。核心病历与已有病史先就绪，随后稳定正式诊断与诊疗路由，允许既有治疗任务与后续 AI 书写候选生成重叠；不新增模型请求、不减少临床字段，不把候选当作诊疗依据。`features/voice-consultation/lib/voiceIntentStreamProtocol.ts` 统一当前请求的分区顺序：对完整连续八行 NDJSON 示例只重排原行，并在系统消息末尾明确当前协议，保留远程提示词的临床规则；用户消息提醒复用同一顺序。本地与 PCIE Server 的 `voiceIntentRecognitionStream` 默认提示词升为 v1.5，已有托管覆盖也在客户端请求边界统一发送顺序。慢病专用流不受影响。
...
慢病复诊核查写入现病史时，`clinical-result/lib/chronicRefillReviewRecordText.ts` 统一校验核查选项正文：只允许简短用药状态、病情控制和症状事实；含具体药名、剂量、频次、用法、疗程、总量或库存推荐信息的片段整段不写入，避免删除药名后留下残缺或改变临床语义。生成计划归一与结果页点击入口共用此规则，点击校验也覆盖已有计划；流式病历或旧快照若在“复诊配药”后拼接详细处方，由 `normalizeChronicRefillHistoryOfPresentIllness` 只保留诊断和复诊目的；复诊核查说明和依据只保留简短药名，去除口服方式、天数、总量和包装信息。兜底“仍按近期方案服用”只写“仍按近期方案服药”。药品事实继续保留在用药史与药品卡核查中，异常选项仍取消药品自动选中，医生手工正文不做全局清洗。

### 女性月经史与婚育史独立链路

`menstrualHistory`（月经史）与 `maritalReproductiveHistory`（婚育史）是独立的可选病历字段，位于个人史与家族史之间。`clinical-result/lib/femaleHistoryEligibility.ts` 以患者性别、完整年龄文本和明确周岁值统一判定资格：仅女性且 `14 <= ageYears < 60` 时开放；月龄、日龄、年龄不明、未满 14 岁和已满 60 岁均关闭。两者共用现有普通语音同次抽取、增量应用、编辑快照、补充重写和字段级回写，不新增模型请求；关闭时模型上下文不携带既有两类病史，结果应用会清空迟到分区与旧缓存，回写入口再次省略。慢病结果构建仅在符合资格时带入当前患者既有两类病史；无明确内容留空，不用正常模板补齐。婚育史记录明确婚育状况、当前妊娠状态及生育事实，不以年龄、已婚、既往孕产次数或月经推断当前妊娠。
...
`recordConfirmedEmrFieldValues.ts` 是症状问诊与普通语音共用的 PHIS 病历字段投影入口。医生选择回写体格检查时，它保留完整 `体格检查` 正文，并复用 `physicalExamVitalTemplate.ts` 将固定 T/P/R/BP 片段及身高、体重、腰围测量槽位剥离：明确生命体征继续投影到 `体温/脉搏/呼吸/收缩压/舒张压`，人体测量值继续保留在完整正文或 PHIS 独立字段中，仅剩余描述性查体正文同时投影到 `其他体格检查/其他体格检查文本`。只有剩余正文非空时才发送这两个字段，避免用空值清除 PHIS 原内容；未选择体格检查时不发送任何相关字段。动态门诊模板仍由 `outpatientEmrStructuredProjection.ts` 按本次 `targetFieldIds` 投影，不扩大模板范围。
