...
`menstrualHistory`（月经史）与 `maritalReproductiveHistory`（婚育史）是独立的可选病历字段，位于个人史与家族史之间。两者共用现有普通语音同次抽取、增量应用、编辑快照、补充重写和字段级回写，不新增模型请求。慢病结果构建必须带入当前患者既有两类病史；无明确内容留空，不用正常模板补齐。患者/就诊切换清理旧值，男性不展示或回写。婚育史记录明确婚育状况、当前妊娠状态及生育事实，不以年龄、已婚、既往孕产次数或月经推断当前妊娠。
