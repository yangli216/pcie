...
普通语音治疗目录准备由 `voiceTreatmentRecommendationGeneration.ts` 分支内编排：药品库存获取与推荐、检查 / 检验实时目录获取与联合推荐同时启动；检查目录等待、为空或失败均不阻塞药品分支回调。检查与检验仍共享一次目录请求与一次模型请求，缺失类型分别回报错误；整体生成在全部分支及结果应用回调完成后收口，原有诊断展示依赖、药品定稿与回写门禁保持不变。
