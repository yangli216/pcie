...
动态门诊模板中的“其他体格检查”字段属于体格检查章节的专用投影：动态分析仍以模型生成字段为基础，但确认前由 `outpatientEmrStructuredProjection.ts` 从已确认的 `physicalExam` 正文中确定性去除体温、脉搏、呼吸、血压等固定生命体征，只将剩余查体内容回填该字段。固定生命体征继续通过 `physicalExamVitalSigns` / 对应 data-id 回写，不能由模型重复拼接或从诊断、候选查体推断。
