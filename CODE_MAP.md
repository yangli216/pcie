...
语音耗时观测：`features/voice-consultation/model/voiceTimingTracker.ts`（轮次隔离、阶段/span、控制台报告）+ `useVoiceTimingPresentation.ts`（DOM 更新与初始结果就绪观测）；录音确认、语音 controller、意图识别和结果页只接入探针，不修改临床流程。
...
| 修改普通语音诊断作用域 / 症状性工作诊断 / 主流 Prompt 上下文 | [ordinaryVoiceDiagnosisGuard.ts](src/features/voice-consultation/lib/ordinaryVoiceDiagnosisGuard.ts) + [voiceIntentStreamProtocol.ts](src/features/voice-consultation/lib/voiceIntentStreamProtocol.ts) + [useVoiceIntentRecognition.ts](src/features/voice-consultation/model/useVoiceIntentRecognition.ts) + [patientMemoryPromptContext.ts](src/features/patient-memory/lib/patientMemoryPromptContext.ts) + [physicalExamGuidance.ts](src/features/clinical-result/lib/physicalExamGuidance.ts) + [diagnosisSuggestionPresentation.ts](src/features/clinical-result/diagnosisSuggestionPresentation.ts) + [DiagnosisRecommendationCard.vue](src/features/consultation-result/ui/DiagnosisRecommendationCard.vue) + [VoiceConsultationNew.vue](src/components/VoiceConsultationNew.vue) + [prompts.ts](src/prompts/prompts.ts)；Prompt 必须区分临床角色、诊断种类、当前就诊证据与历史上下文，并携带权威完整年龄；纯规则过滤 `history_only / risk_modifier`，待鉴别只保留可解释当前主诉的病因项；无病因性正式诊断时最多提升一项已匹配 R 类标准编码的本次肯定症状为工作诊断，并把 AI 诊疗路由限制为检查 / 检验。托管 Prompt 旧顺序只通过单份 system 运行时约束修正，user 不重复完整协议；长期记忆压缩为去重名称线索，基础查体由客户端补齐，模型只生成基础库外增量。门禁只作用于普通 `voice`，不得影响症状问诊或医生已确认慢病范围的 `chronic-refill` |
...
| 修改固定病史模板槽位 / PHIS 精确回写 | [historyRecordTemplates.ts](src/features/clinical-result/historyRecordTemplates.ts) + [recordConfirmedEmrFieldValues.ts](src/features/clinical-result/recordConfirmedEmrFieldValues.ts) + [outpatientRecord.ts](src/features/clinical-result/outpatientRecord.ts) + [recordConfirmedPayload.ts](src/features/clinical-result/recordConfirmedPayload.ts) + [clinicalRecordFactConfirmation.ts](src/features/clinical-result/clinicalRecordFactConfirmation.ts) + [api.md](api.md)；固定 `{否认}/{有}` 槽位解析、稳定 `slotKey`、完整槽位值和阳性变化清单统一归模板 helper，结果页按“根据上下文修正”展示阳性变化，一键回写只展开医生已选字段；`recordConfirmedEmrFieldValues.ts` 把已选标准正文、既往史/个人史/家族史全部结构槽位和生命体征展开为可直接传给 PHIS `medEleDatabinds` 的 `data-id → 绑定值`，字典字段使用真实 `BINDINGDATA.VALUE` |
...
| 修改医学数据匹配 / HIS 出站标准服务 | [medicalData.ts](src/services/medicalData.ts) + [useReceptionController.ts](src/app/events/useReceptionController.ts) + [his/HisAdapter.ts](src/services/his/HisAdapter.ts) + [his/types.ts](src/services/his/types.ts) + [his/PhisHisAdapter.ts](src/services/his/PhisHisAdapter.ts) + [his/phisMedicineServiceCode.ts](src/services/his/phisMedicineServiceCode.ts) + [his/MockHisAdapter.ts](src/services/his/MockHisAdapter.ts) + [his/registry.ts](src/services/his/registry.ts) + [hisService.ts](src/services/hisService.ts) + [recordConfirmedPayload.ts](src/features/clinical-result/recordConfirmedPayload.ts) + [availableMedicineInventory.ts](src/features/clinical-result/api/availableMedicineInventory.ts) + [medical_catalog.rs](src-tauri/src/commands/medical_catalog.rs)；检查 / 检验目录必须在每次接诊开始时通过 `queryAvailableExamLabItems` 实时获取、同次接诊内存复用、结束时失效，不得从 SQLite 恢复或在失败时回退历史目录；重点核对 `jsonField.idLisCategory`、执行科室、部位、互认编码和 `raw` 是否完整进入匹配与回写。PHIS 药品分类以 `HiBdMed.sdMed` 为事实源，`1 -> sdSrv 11`、`2 -> sdSrv 12`；实时目录进入 Adapter 时映射，最终回写优先用药品详情 `raw.sdMed` 覆盖缓存恢复的通用默认值。诊断与非检查检验诊疗项目仍按机构+租户隔离，药品按机构+租户+药房 `storeId` 隔离；有效库存目录必须经 `HisAdapter.fetchAvailableMedicineInventory()` 获取，PHIS 侧经 `api/phis.aiAdapterService/queryInvSubList` 调用院端适配入口并按 `idMedPro` 合并批次，AI 缓存按机构+租户+药房隔离，Prompt 投影必须按清洗后“名称 + 规格”去重且不能反向合并真实 `productId` 库存，库存校验单价按 `storeId + idMedPro` 从目录合并项解析且不得回退药品详情；住院上下文是否使用 PHIS `idAdsn`（中性 `admissionId`）或 `patientId / inpatientVisitId / encounterId / inpatientNo` 锚定同患者多次住院，住院病历 AI 上下文是否只经 `fetchInpatientEmrContext` / PHIS `buildContext` 聚合入口返回，以及服务端暂不可用时既有缓存仍可恢复 |
...
| **aiTrace.ts** | ~200 | AI 调用链路缓存 + AI 代理结构化日志桥：补齐 traceId、scene、sourceModule 以及业务发起方；`llm.ts` 将业务动作/标题同步到服务端代理请求，使后台实测耗时可按具体调用定位 |
...
慢病核查正文门禁：`features/clinical-result/lib/chronicRefillReviewRecordText.ts` 是药名/处方明细校验纯规则；`normalizeChronicRefillHistoryOfPresentIllness` 清理流式病历或旧快照中“复诊配药”后的详细处方，`chronicRefillConfirmation.ts` 归一复诊说明/依据，`useChronicRefillReview.ts` 负责点击写入并尊重医生手工编辑，防止初始现病史清理后被核查选项再次带入历史药品。

女性病史：`maritalReproductiveHistory` 与 `menstrualHistory` 同路独立传递；`patientContext.ts` 提取两类既有病史，`chronicRefillRecord.ts` 补齐慢病患者病史输入；`clinical-result/lib/maritalReproductiveHistory.ts` 解析明确婚育/当前妊娠字典事实，由 `recordConfirmedEmrFieldValues.ts` 输出 PHIS data-id。语音流、结果页快照、重写、部分回写和动态模板映射均保留两字段。

女性字段编辑控制器：`features/consultation-result/model/useFemaleHistoryFields.ts` 统一月经史/婚育史的缓存恢复、逐字段流式基线保护与非女性清理；共享结果页仅编排，不为两字段新增全局状态。
