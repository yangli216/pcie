...
14. **多慢病复诊范围门禁**：患者历史存在多个慢病时，复诊核查项、本次主诉/现病史、初始诊断和用药推荐必须只消费医生已确认的 scoped candidate；未选慢病的诊断、就诊和药品不得混入本次诊疗范围。既往史是患者事实，不受本次配药选择裁剪：必须合并同患者完整历史慢病与 HIS 既往史，已知慢病不得因未选而被模板或 AI 写成否认；该历史信息不得自动扩充本次诊断、核查项或药品。胶囊中的诊断勾选就是本次范围确认，不得再增加逐药确认步骤。同次就诊含多慢病且 HIS 无处方-诊断归属关系时，部分选中不得整单沿用；只能自动纳入经白名单归一、由慢病同一次生成流 `medication_scope` 明确归入所选诊断且置信度为 high / medium 的历史药品。禁止恢复接诊阶段独立药品归类请求或胶囊提交等待；胶囊不得展示 AI 识别、匹配数量、成功、失败或无匹配状态。`medication_scope` 只能引用请求内既有稳定药品 item id 和候选慢病 id，低置信、未归类、越权新增或流式失败的药品不得进入 scoped candidate，但仍保留在完整处方历史中供结果页事实核查。自动归类药品进入结果页后仍须经过共享药品定稿流水线并允许医生取消或调整，未经结果页最终确认不得回写。
...
26. **现有信息用药评估门禁**：医生主动继续推荐仅开放当前普通语音/症状问诊的药品分支，不修改自动诊疗路由，不影响检查建议或医生已有医嘱。症状性工作诊断仅允许对症候选；缺少关键依据的药品只展示暂缓原因，不进入可选择药品或回写；紧急转诊、仅明确医嘱和仅待鉴别方向不开放入口。请求必须携带当前安全相关病史，允许空结果，不凑固定药品数量；新药定稿和库存核查后默认未选。病例/患者/就诊/轮次/主诊断变化须拦截迟到结果，不得把医生请求写成患者拒检或结果正常。
...
查体基础项目只按本次就诊选取，默认肺部书写候选按医生确认使用“双肺呼吸音粗”，不得解释为正常或已听诊；本次明确的正常/异常结果均不得被默认候选覆盖。新增测量值不得由模板推断，身高/体重/腰围仅为正文槽位。查体补充必须保留 AI 来源与医生编辑/移除状态，不得作为诊断或用药的已核实依据；语音与慢病继续同次流，慢病不得补发通用候选请求。
